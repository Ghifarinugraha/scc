<div class="sm:px-6 lg:px-24">
    <x-bg-card>
        <div>
            ...
            @endif
            @if ($currentPage == 2)
            <form class="m-4" action="POST" wire:submit.prevent="register">
                @csrf
                <div class="mt-6 grid grid-cols-1 sm:grid-cols-2">
                    <div class="sm:col-span-full mb-6">
                        ...
                        <x-text-input  id="namaLengkap" placeholder="Nama" class="block mt-1 w-full p-2.5 border-gray-300  " type="text" name="nama_lengkap" :value="old('text')" wire:model="nama_lengkap" />
                 ...
                 <div class="flex items-center justify-between px-4 py-2 w-full bg-gray-50 text-right sm:px-6">
                    ...
                    @endif
                    @if ($currentPage == 3)
                        <button wire:click="submit" type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#FF852F] hover:bg-[#ff7636] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#FF852F]">Submit</button>
                    @else
